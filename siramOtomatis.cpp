// --- Konfigurasi Pin ---
const int pinSensor = A0;  // Pin analog untuk membaca sensor kapasitif
const int pinRelay  = 12;  // Pin digital untuk mengontrol relay pompa

// --- Batas Kelembapan (Threshold) ---
// Catatan: Sensor kapasitif biasanya bernilai TINGGI saat KERING dan RENDAH saat BASAH.
const int batasKering = 600; 

void setup() {
  // Memulai komunikasi serial untuk memantau nilai di Serial Monitor
  Serial.begin(9600);
...
void loop() {
  ...
  if (nilaiKelembapan > batasKering) {
    // Jika tanah kering (nilai lebih besar dari batas)
    Serial.println("Status: Tanah Kering -> POMPA MENYALA");
    digitalWrite(pinRelay, LOW); // Nyalakan relay (sesuaikan HIGH/LOW dengan jenis relaymu)
  } else {
  ...
  // Jeda 2 detik sebelum membaca ulang agar sistem tidak terlalu berat
  delay(2000); 
}