...
5. Kabel Jumper & Adaptor Daya/Baterai (Eksternal untuk pompa)

## Skema Rangkaian (Wiring)

Berikut adalah diagram ASCII untuk koneksi antar modul perangkat keras:

    [ Arduino Uno / Nano ]
              |
              |--- 5V  ----------+---------[ VCC ] (Soil Moisture Sensor)
              |                  |
              |                  +---------[ VCC ] (Relay Module)
              |
              ...
              |                  +---------[ GND ] (Relay Module)
              |
              |--- A0  <-------------------[ AOUT] (Soil Moisture Sensor)
              |
              |--- D12 >-------------------[ IN  ] (Relay Module)
                                             |
                                           [COM]------( + Power Supply Pompa )
                                           [NO ]------( + ) [ Pompa Air ]
                                                      ( - )------( - Power Supply Pompa )
...
> **Catatan Pompa:** Mikrokontroler biasanya tidak mampu menyuplai arus yang cukup untuk pompa air. Gunakan sumber daya terpisah (baterai/adaptor) untuk pompa. Hubungkan kutub positif (+) sumber daya eksternal ke terminal **COM** relay, dan kabel positif (+) pompa ke terminal **NO (Normally Open)**. Kutub negatif (-) pompa langsung dihubungkan ke negatif (-) sumber daya eksternal.

## Cara Penggunaan

1. Rangkai seluruh komponen sesuai dengan diagram ASCII dan tabel wiring di atas.
